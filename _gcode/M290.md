...
long:
  ...
  - Note that the axis being babystepped needs to be already enabled (in motion or actively holding position) at the time of babystepping unless `BABYSTEP_ALWAYS_AVAILABLE` is enabled, for example if your configuration specifies `#define DISABLE_Z true` then you should also specify `#define BABYSTEP_ALWAYS_AVAILABLE` otherwise babystepping would only work during the moment that the Z axis is being moved.
